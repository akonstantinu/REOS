...
//y=2.56x-3.37 FPS
  ...
  double enthalpy(vector<double> p,vector<double> en,double h0,int num,int data){
    ...
    double dx=(p.at(num)-p.at(data))/((num-data));
    double integr=0;
    double press=p.at(0);
...
void EOSmultTOV(){
  ...
  while(fileNum<num){
    ...
    //Get e,p,h,d Begins
    double tempE,tempH;
    vector<double> vEnergyDensity,vPressure,vEnthalpy,vDensity,dim_p,dim_e;
    ...
    for(int i=0;i<v_h.size();i++){
      ...
      vEnthalpy.push_back(v_h.at(i)/(c*c));
      dim_p.push_back(v_p.at(i));//dimensional pressure and energy
      dim_e.push_back(v_e.at(i));
    }
    for(int i=v_d.size()-1;i>v_h.size();i--){
      vPressure.push_back(v_p.at(i)*kappa*G/(c*c*c*c));
      vDensity.push_back(v_d.at(i)*kappa*G/(c*c));
      tempE=fp.e(v_d.at(i)*(G*kappa)/(c*c));
      vEnergyDensity.push_back(tempE);
      dim_p.push_back(v_p.at(i));
      dim_e.push_back(tempE*c*c/(kappa*G));
      //Finds N by using the politropic equation
      vEnthalpy.push_back(TMath::Log(fp.h(v_d.at(i)*kappa*G/(c*c))));
    }

    //Finds the N by integration
    /*
    for(int i=v_h.size();i<vDensity.size();i++){
       tempH=fp.enthalpy(dim_p,dim_e,v_h.at(v_h.size()-2),i,v_h.size()-1);
    ...
    for(int i=0;i<vDensity.size();i++){
      output<<vEnergyDensity.at(i)/(kappa*G/(c*c))<<" "<<vPressure.at(i)/(kappa*G/(c*c*c*c))<<" "<<vEnthalpy.at(i)*(c*c)<<" "<<vDensity.at(i)/(Mb)/(kappa*G/(c*c))<<" "<<endl;
      
      // cout<<vEnergyDensity.at(i)/(kappa*G/(c*c))<<" "<<vPressure.at(i)/(kappa*G/(c*c*c*c))<<" "<<vEnthalpy.at(i)*(c*c)<<" "<<vDensity.at(i)/(Mb)/(kappa*G/(c*c))<<" "<< TMath::Log((vEnergyDensity.at(i)+vPressure.at(i))/vDensity.at(i))*c*c<<" "<<(vEnergyDensity.at(i)/(kappa*G/(c*c))+vPressure.at(i)/(c*c*kappa*G/(c*c*c*c)))*TMath::Exp(-vEnthalpy.at(i)/(c*c*kappa*G/(c*c)))/(Mb)<<" "<<vEnergyDensity.at(i)/vDensity.at(i)<<endl;
    }
