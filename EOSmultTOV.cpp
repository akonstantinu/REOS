...
public:
  ...
  double e(double d){//energy density
    ...
    a2=-1+K2/(Gamma2-1)*TMath::Power(rho1,Gamma2-1)+(1+a1)+K1/(Gamma1-1)/rho1*TMath::Power(rho1,Gamma1);
    a3=-1+K3/(Gamma3-1)*TMath::Power(rho2,Gamma3-1)+(1+a2)+K2/(Gamma2-1)/rho2*TMath::Power(rho2,Gamma2);
  ...
  double enthalpy(vector<double> p,vector<double> en,double h0,int num,int data){
    ...
    double xold,xnew;
    double integr=0;
    integr=f(p.at(data-1),en.at(data-1))*(p.at(data)-p.at(data-1))/3.+f(p.at(num),en.at(num))*(p.at(num)-p.at(num-1))/3.;
    for(int i=data-1;i<num;i++){
      if(i%3==0){
	integr+=2.*f(p.at(i),en.at(i))*(p.at(i)-p.at(i-1))*3./8.;
      }
      else{
	integr+=3.*f(p.at(i),en.at(i))*(p.at(i)-p.at(i-1))*3./8.;
      }
    }

    integr=h0+integr*(kappa*G)/(c*c);
  ...
  double maxMass(vector<double> v_d,vector<double> v_p,vector<double> v_e){
	...
	*/
    ...
    //return (r*TMath::Power(kappa,0.5))/100000.;
  }
...
void EOSmultTOV(){
  ...
  while(fileNum<num){
	...
	v_d.push_back(stod(a4)*1.66*TMath::Power(10,-24));
      }
      count++;
    }
    count=0;
    ...
    //EOS Begining
    double stepsize=d0/10000.;//TMath::Power(10,12)*kappa*G/(c*c);
    double p0=fp.pressure(d0);
    double d,p;
    d=d0;
    p=p0;
    ...
    //Get e,p,h,d Begins
    double tempE,tempH;
    vector<double> vEnergyDensity,vPressure,vEnthalpy,vDensity;
    
    for(int i=0;i<v_h.size();i++){
      vPressure.push_back(v_p.at(i)*kappa*G/(c*c*c*c));
      vDensity.push_back(v_d.at(i)*kappa*G/(c*c));
      vEnergyDensity.push_back(v_e.at(i)*kappa*G/(c*c));
      vEnthalpy.push_back(v_h.at(i)*kappa*G/(c*c));
    }
    for(int i=v_d.size()-1;i>v_h.size();i--){
      vPressure.push_back(v_p.at(i)*kappa*G/(c*c*c*c));
      vDensity.push_back(v_d.at(i)*kappa*G/(c*c));
    }

    for(int i=v_h.size();i<vDensity.size();i++){
      tempE=fp.e(vDensity.at(i));
      vEnergyDensity.push_back(tempE);
      tempH=fp.enthalpy(vPressure,vEnergyDensity,v_h.at(v_h.size()-1),i,v_h.size());
      vEnthalpy.push_back(tempH);
    }
    //Get e,p,h,d ENDS

    if(fp.maxMass(vDensity,vPressure,vEnergyDensity)/(Mo*G)>1.97) continue;

    
    output<<vEnergyDensity.size()/*-data*/<<endl;//remove data when suurface included
    for(int i=0;i<vDensity.size();i++){
      output<<vEnergyDensity.at(i)/(kappa*G/(c*c))<<" "<<vPressure.at(i)/(kappa*G/(c*c*c*c))<<" "<<vEnthalpy.at(i)/(kappa*G/(c*c))<<" "<<vDensity.at(i)/(1.66*TMath::Power(10,-24))/(kappa*G/(c*c))<<" "<<endl;
