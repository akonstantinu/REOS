...
void TOV_enthalpy(){
  ...
  vector<double> maxm1={1.7413,1.7414,1.7474,1.7568,1.77719,1.7855,1.7914,1.7985,1.7904,1.78810,1.7744,1.7519,1.7088,1.6666,1.6259,1.5937,1.5366,1.4921,1.4676,1.444,1.394,1.3698,1.3272,1.311,1.2611,1.1924,1.15};
 
  vector<double> maxr1= {8.1916,8.1928,8.2413,8.3236,8.4779,8.6574,8.7659,9.1230,9.3909,9.43,9.6083,9.8067,10.075,10.274,10.434,10.545,10.719,10.838,10.898,10.952,11.05,11.086,11.135,11.149,11.188,11.234,11.261};
  ...
  while( getline(myinputfile,line) ){
    if(count==0){
      ...
      v_eta.push_back(stod(a3)*kappa*G/(c*c)-1);
  ...
  count=0;
  for(int dD=0;dD<1;dD++){//the loop is for different central densities 
    double stepsize=-0.001;
    double eta0=h((1.91407*1.66*TMath::Power(10,15)-dD*TMath::Power(10,15)*0.05)*kappa*G/(c*c)-1);
    double d0=(1.91407*1.66*TMath::Power(10,15)-dD*TMath::Power(10,15)*0.05)*kappa*G/(c*c);
  ...
  TGraph* m_r =new TGraph(v_m.size(),&v_r.at(0),&v_m.at(0));
